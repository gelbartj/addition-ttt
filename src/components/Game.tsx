...
import { debugLog, GameChoice as GameType } from "../App";
import {
  addBoard,
  emptyStatus,
  makeBoardNums,
...
export const Game: React.FC<GameProps> = ({
      ...
      if (state.winner && state.winSquares) {
        return `🎉 Game over. ${
          !isMultiplayer
            ...
            ? state.winner === "X"
              ? "You win!"
              : state.oUsername + " wins"
            : state.winner === "O"
            ? "You win!"
            : state.xUsername + " wins"
        } 🎉`;
      }
      return "";
    },
  ...
  const getMovesResult = useCallback(
    (moves: (number | null)[]) => {
      let movesSum =
        moves.length === 2 && moves[0] !== null && moves[1] !== null
          ? moves[0] + moves[1]
          : undefined;
  ...
  const getIsYourTurn = (currPlayer: Players) =>
    !isMultiplayer ||
    (isMultiplayer &&
  ...
  function isUpdateAction(action: Action): action is UpdateAction {
    return (
      action.type === UPDATE_STATE_VALUE &&
      (action as UpdateAction).payload !== undefined
    );
  }
  ...
  function reducer(
    ...
    if (isUpdateAction(action)) {
      debugLog("warn", "---------Updating values: ", action.payload);
      return {
        ...state,
        ...action.payload,
      };
      ...
      switch (action.type) {
        case actions.ADD_CLICKED_MOVE:
          debugLog("warn", "---------Dispatch: add clicked move");
          return {
            ...state,
        ...
        case actions.RESET_GAME:
          ...
          debugLog("Initial: ", initialGameState);
          const resetState = {
            ...initialGameState,
  ...
  useEffect(() => {
    ...
    const syncDB = async (state: typeof initialGameState) => {
      ...
      debugLog("Preparing to send update to GraphQL");
      ...
      debugLog("=============", "Sending update to GraphQL...", input);
      return (API.graphql(
        graphqlOperation(updateGame, { input: input })
      ) as Promise<GraphQLResult<UpdateGameMutation>>)
        .then((e) => {
          if (e) debugLog("Successfully updated DB: ", e);
        })
        .catch((e) =>
          debugLog("error", "Error updating graphQL database: ", e)
        );
    };
  ...
  const deserialized = useMemo(() => {
    debugLog("receivedGameState changed: ", receivedGameState);
    return receivedGameState ? qlToState(receivedGameState) : null;
  }, [receivedGameState]);

  useEffect(() => {
    if (deserialized !== null) {
      debugLog("Deserialize changed: ", deserialized);
      let gameOverMsg = "";
      ...
      dispatch({
        ...
        payload: { ...deserialized, currError: gameOverMsg },
      });
    }
  ...
  function getBoardInstructions(
    ...
    if (state.currMoves.length === 2 && state.moveCount === 0) {
      ...
      state.moveCount === 1 &&
      !noMoves &&
      (!isMultiplayer || !getIsYourTurn(state.currPlayer))
    ) {
      ...
      for (let col = 0; col < state.activeSquares[0].length; ++col) {
        if (state.activeSquares[row][col]) {
          ...
          if (winChain.length >= 4) {
            ...
            return true; */
            return [state.activeSquares[row][col]!, winChain];
          }
        }
      }
    }
    ...
    <>
      ...
      {gameObj && (
        <div className="challengers">
          ...
          <span className="username">{gameState.oUsername}</span>
        </div>
      )}
